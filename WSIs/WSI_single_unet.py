dataname="WSI_single_A1"
extention = '_annotations'
ignore_index = -100
gpuid= 0
...
from models.loss import CustomSoftDiceLoss
from models.utils import iou, asMinutes, timeSince
import os
...
class Dataset(object):
    def __init__(self, fname, extention='_annotations', img_transform=None, mask_transform = None, edge_weight= False):
    ...
    def __getitem__(self, index):
        ...
        path[0] = '/'
        basename = path[-3]
        if self.extention == '_annotations':
            path[-2] = basename+'_annotations'
        else:
            path[-2] = self.extention
        ...
        if self.mask_transform is not None:
            ...
            mask1_new = np.asarray(mask1_new)[:,:,0].squeeze()
            random.seed(seed)

        return img_new, mask1_new, path[-1]
...
for k in range(5):
    ...
    dataLoader={}
    for phase in phases: 
        dataset[phase]=Dataset(f"/home/eikthedragonslayer/Desktop/CVS_data/kidney/{phase}_{k}.txt", extention=extention, img_transform=img_transform, mask_transform = mask_transform)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in phases:
            ...
            for ii , (X, y1, name) in enumerate(dataLoader[phase]): 
                X = X.to(device)  # [Nbatch, 3, H, W]
                y1 = y1.type('torch.LongTensor').to(device)  # [Nbatch, H, W] with class indices (0, 1)
                with torch.set_grad_enabled(phase == 'train'): 
                                                               
                    prediction1 = model(X)  # [N, Nclass, H, W]
                    loss_matrix1 = criterion(prediction1, y1)
                    ...
                    if phase in validation_phases: #if this phase is part of validation, compute confusion matrix
                        p=prediction1[:,:,:,:].detach().cpu().numpy()
