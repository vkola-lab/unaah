...
def build_parser():
    ...
    # Either or arguments
    parser.add_argument("--data_path", default='', type=str, help='The absolute path of directory containing the WSIs', metavar='input_image')
    ...
    group_exec.add_argument(
        ...
        type=int,
        default=75,
        help='Integer indicating the overlap on each edge of the patch. A value of Overlap will produce tiles of size P+(2*O) x P+(2*O).')
    group_exec.add_argument(
        '--method',
    ...
    group_output.add_argument(
        ...
        action='store_true',
        default=False,
        help='Save the annotation tiles corresponding to the saved patches')
    group_output.add_argument(
        '--save-blank',
        action='store_true',
        default=False,
...
def check_arguments(args):
    ...
    args.multiple = True # Denotes, there is a directory of WSIs for processing.

    # Argument checking for graph segmentation
    if (args.svs != '' and args.data_path != ''):
        raise ValueError("Provide Either single svs or the datapath with multiple WSIs. Not both.")
